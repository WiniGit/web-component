...
export function Winicon({ id, src, link, className, style, size, color, alt, onClick }: WiniconProps) {
    ...
    return <div id={id} onClick={onClick} className={`${styles['wini-icon']} ${svgData? "" : "skeleton-loading"} ${onClick ? styles['clickable'] : ''} ${className ?? ''} ${src ? src.split("/").map((e, i) => i === 0 ? `${e}-icon` : e.replace(" ", "-")).join(" ") : ''}${link ? ' link-icon' : ""}`} style={(style ? { ...style, '--size': size, '--color': color } : { '--size': size, '--color': color }) as any} dangerouslySetInnerHTML={{ __html: svgData ?? '' }} />
}
